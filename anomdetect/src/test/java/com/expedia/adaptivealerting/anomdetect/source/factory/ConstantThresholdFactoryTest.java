...
@Slf4j
public class ConstantThresholdFactoryTest extends AbstractDetectorFactoryTest {
    private static final double TOLERANCE = 0.001;

    @Test(expected = RuntimeException.class)
    public void testInit_nullDocument() {
        new ConstantThresholdFactory(null);
    }

    @Test
    public void testBuildDetector() {
        val document = readDocument("constant-threshold");
        val factoryUnderTest = new ConstantThresholdFactory(document);
    ...
    public void testBuildDetector_invalidParams() {
        ...
        val factory = new ConstantThresholdFactory(document);
