...
public class LegacyDetectorFactory {
    ...
    public Detector createDetector(UUID uuid, DetectorDocument detectorDocument) {
        ...
        if (CONSTANT_THRESHOLD.equals(detectorType)) {
            ...
            detector = new ConstantThresholdOutlierDetector(uuid, params);
        } else if (CUSUM.equals(detectorType)) {
            val params = toParams(detectorParams, CusumParams.class).toNewParams();
            detector = new CusumOutlierDetector(uuid, params);
            ...
            detector = new IndividualsOutlierDetector(uuid, toParams(detectorParams, IndividualsOutlierDetector.Params.class));
    ...
    public Detector createEwmaDetector(UUID uuid, EwmaParams params) {
        ...
        return new ForecastingOutlierDetector(uuid, pointForecaster, intervalForecaster, AnomalyType.TWO_TAILED);
    }
