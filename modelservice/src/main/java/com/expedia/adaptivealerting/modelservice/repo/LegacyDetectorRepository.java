...
@RepositoryRestResource(excerptProjection = InlineType.class, collectionResourceRel = "detectors", path = "detectors")
public interface LegacyDetectorRepository extends PagingAndSortingRepository<LegacyDetector, Long> {
     ...
     */
    List<LegacyDetector> findByUuid(@Param("uuid") String uuid);
     ...
     */
    List<LegacyDetector> findByCreatedBy(@Param("user") String user);
     ...
     */
    @Query("select mmm.detector from LegacyMetricDetectorMapping mmm where mmm.metric.hash = :hash")
    List<LegacyDetector> findByMetricHash(@Param("hash") String hash);
    ...
    List<LegacyDetector> getLastUpdatedDetectors(@Param("interval") int interval);
    ...
    @Modifying
    @Transactional
    @Query("update LegacyDetector d set d.enabled = :enabled where d.uuid = :uuid")
